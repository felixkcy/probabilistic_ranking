...
def get_stats(x, burn_in=100):
    x = x[burn_in:]
    return np.mean(x, axis=-1), np.std(x, axis=-1)


if __name__ == '__main__':
    data = loadmat('tennis_data.mat')
    skill_samples1 = gibbs_sample(data['G'] - 1, data['W'].shape[0], 600)
    # skill_samples2 = gibbs_sample(data['G'] - 1, data['W'].shape[0], 1100)
    print(get_stats(skill_samples1))
    # print(get_stats(skill_samples2))